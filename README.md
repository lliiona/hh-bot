...
2. Установите зависимости: `pip install aiogram sqlalchemy asyncpg httpx apscheduler python-dotenv`
3. Скопируйте `.env.example` в `.env` и заполните переменные:
   - `BOT_TOKEN`: токен Telegram бота от @BotFather
   - `DATABASE_URL`: URL PostgreSQL базы данных (используйте Neon или локальную)
   - `LLM_BASE_URL`, `LLM_API_KEY`, `LLM_MODEL`: настройки для OpenAI-compatible LLM
